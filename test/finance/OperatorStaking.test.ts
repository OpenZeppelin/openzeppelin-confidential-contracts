...
async function fixture() {
  ...
  const initializerCall = (
    await impl.initialize.populateTransaction('ProtocolStakingToken', 'PST', '1', baseToken.target, protocolAdmin)
  ).data;
  ...
  const operatorStaking = (await ethers.deployContract(
    '$OperatorStaking',
    [await protocolStaking.getAddress(), operator, 'OperatorStakingToken', 'OST'],
    operator,
  ...
  describe('Holder staking and unstaking', function () {
    it('should claim holder rewards if operator staked', async function () {
      ...
      for (const holder of holders) {
        ...
        await baseToken.connect(holder).approve(await operatorStaking.getAddress(), amount);
