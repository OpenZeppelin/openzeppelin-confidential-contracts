...
abstract contract ERC7984Freezable is ERC7984 {
    ...
    function _checkFrozenBalance(address from, euint64 requestedTransferAmount) internal virtual returns (euint64) {
        euint64 unfrozen = confidentialAvailable(from);
        return FHE.select(FHE.le(requestedTransferAmount, unfrozen), requestedTransferAmount, FHE.asEuint64(0));
    }
    ...
    function _update(address from, address to, euint64 encryptedAmount) internal virtual override returns (euint64) {
        if (from != address(0)) {
            encryptedAmount = _checkFrozenBalance(from, encryptedAmount);
        }
