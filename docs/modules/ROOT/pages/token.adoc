...
Swapping from a confidential `ERC7984` to another confidential `ERC7984` is a bit more complex although quite simple given the usage of the `TFHE` library. For the sake of the example, we will swap from `fromToken` to `toToken` with a 1:1 exchange rate.
